import alt from 'alt-client';
import game from 'natives';

import { ModuleBase } from "../utils/models/baseModels/module.base.mjs";

export default new class PropSyncModule extends ModuleBase {
  public tempProp: number;
  public propData: { propName: string, bone: number, posX: number, posY: number, posZ: number, rotX: number, rotY: number, rotZ: number } | null;

  constructor() {
    super('PropSyncModule');

    this.tempProp = -1;
    this.propData = null;

    alt.onServer("Client:PropSyncModule:AddProp", this.addProp.bind(this));
    alt.onServer("Client:PropSyncModule:Clear", this.clearProps.bind(this));

    alt.everyTick(this.tick.bind(this));
  }

  private addProp(propName: string, bone: number, posX: number, posY: number, posZ: number, rotX: number, rotY: number, rotZ: number): void {
    const player = alt.Player.local;

    if (this.tempProp != -1) {
      var _prop = alt.Object.all.find(x => x.id == this.tempProp);
      _prop.destroy();
      this.tempProp = -1;
    }

    const prop = game.createObject(game.getHashKey(propName), player.pos.x, player.pos.y, player.pos.z, false, true, false);
    if (prop == -1) return;

    game.attachEntityToEntity(prop, player.scriptID, game.getPedBoneIndex(player.scriptID, bone), posX, posY, posZ, rotX, rotY, rotZ, false, false, false, true, 2, true, false);
    this.tempProp = prop;
    this.propData = {
      propName: propName,
      bone: bone,
      posX: posX,
      posY: posY,
      posZ: posZ,
      rotX: rotX,
      rotY: rotY,
      rotZ: rotZ
    };
  }

  private clearProps(): void {
    if (this.tempProp == null) return;

    game.deleteObject(this.tempProp);
    game.detachEntity(this.tempProp, true, true);

    this.tempProp = null;
    this.propData = null;
  }

  private tick(): void {
    if (this.tempProp != -1) {
      const player = alt.Player.local;
      game.attachEntityToEntity(this.tempProp, player.scriptID, game.getPedBoneIndex(player.scriptID, this.propData.bone), this.propData.posX, this.propData.posY, this.propData.posZ, this.propData.rotX, this.propData.posY, this.propData.posZ, false, false, false, true, 2, true, false);
    } else {
      game.deleteObject(this.tempProp);
      game.detachEntity(this.tempProp, true, true);
      this.propData = null;
    }
  }
}